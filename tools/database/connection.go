...
func NewConfiguration(host, user, password, database string, port int, sslMode string) DatabaseCfg {
	return DatabaseCfg{
		DBHost:     host,
		DBUser:     user,
		DBPassword: password,
...
type Connection interface {
	Connect(cfg DatabaseCfg) (Conn, error)
}
...
func NewConnection(driver Connection, cfg DatabaseCfg) (*DBWrapper, error) {
	var err error
	var conn Conn
...
// func NewRedisConnection(ctx context.Context, cfg storify.Configuration) *redis.Client {
...
// Is a simple Migrate wrapper function using AutoMigrate from gorm but adding some information output
// to add in the wrapping function
